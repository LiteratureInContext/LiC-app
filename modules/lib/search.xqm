...
declare %templates:wrap function search:search-data($node as node(), $model as map(*), $collection as xs:string?){
    ...
    (:
        (if(request:get-parameter('clear', '') = 'clear') then 
            session:clear()
        else (),
        if(empty($queryExpr) or $queryExpr = "") then
            let $cached := session:get-attribute("search.Srophe")
            return
                map {
                    "hits" := $cached,
                    "query" := session:get-attribute("search.Srophe.query")
                }      
        else 
            let $hits := data:search($collection)
            let $store := (
                session:set-attribute("search.Srophe", $hits),
                session:set-attribute("search.Srophe.query", $queryExpr)
            )
            return
                map {
                    "hits" := $hits,
